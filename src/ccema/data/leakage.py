...
def layer1_audit(enc: Encounter) -> Layer1Report:
    ...
    # L1a — working diagnosis must not appear verbatim
    # Use case-insensitive word-boundary match. A short generic word like
    # "pain" alone would be too noisy, but real working diagnoses are
    # rarely that generic; reviewers should triage flagged cases manually.
    if enc.working_diagnosis:
        wd = enc.working_diagnosis.strip()
        if wd:
            pattern = r"\b" + re.escape(wd) + r"\b"
            if re.search(pattern, enc.x, flags=re.IGNORECASE):
                rep.leaked_dx = True
